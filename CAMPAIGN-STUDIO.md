# Campaign Skin Studio

`/studio` 是现有活动 H5 的换肤配置工具。它复用同一个
`CampaignExperience` 渲染器，因此配置预览与活动页不会维护两套 UI。

## 使用流程

1. 从“夏天马上顺”或“夏日夜食”创建方案，也可以复制当前方案。
2. 上传 Hero 图片或视频，调整 `cover / contain` 与焦点位置。
3. 修改核心品牌色、活动文案、9 张卡片和 4 档奖励。
4. 在中间的 375 × 875px（9:21）手机窗口实时检查结果。
5. 点击“应用到活动页”，刷新活动页查看当前浏览器中的效果。
6. 导出 JSON 作为主题包备份或交付文件。

## 编辑器结构

右侧检查器不再使用“页面文案”或“高级素材”这类跨模块分组，而是与 H5
从上到下保持一致：

1. 全局设置：主题基础、品牌配色。
2. 首焦与主操作：Hero、地图、主题 Tab、主按钮。
3. 集卡与奖励：卡册、9 张卡片、4 档奖励、货架与卡框皮肤。
4. 副玩法卡。
5. 任务区与 5 个任务。
6. 话题与 3 张灵感卡。
7. 内容发现与 4 张内容卡。
8. 更多精彩活动的 2 个 Banner 位。

素材、文案和局部高级样式都放在其所属模块内；品牌语义色保留为全局设置。

## 画布操作

- 中间区域是无滚动条的设计画布，手机内容不会撑开配置平台。
- 点击“缩小 / 放大 / 适应画布”调整预览比例。
- 按住空格并拖动鼠标可以平移手机画布。
- `Ctrl/⌘ + 滚轮`缩放画布。
- 普通滚轮落在手机内时只滚动 H5；手机滚动条保持隐藏。
- 整个平台固定在当前视口，只有右侧检查器显示滚动条。

配置草稿与玩家活动进度使用不同的存储键：

- 配置草稿：`campaign-studio-drafts-v1`
- 当前应用皮肤：`campaign-active-skin-v1`
- 玩家进度：`summer-campaign-multitheme-v2`

重置玩家体验不会删除皮肤配置，恢复主题默认值也不会清空玩家进度。

## 当前模板约束

第一版以稳定换肤为目标，页面几何不进入普通配置：

- Hero 媒体槽：375 × 500
- Studio 手机画布：375 × 875（9:21）
- 卡片：9 张
- 奖励：4 档
- 任务：5 个
- 话题标签：6 个
- 灵感卡：3 个
- 内容卡：4 个
- 活动 Banner：2 个

配置器会检查 Hero、卡片数量、奖励数量、标签数量、Banner 数量和奖励门槛。
若要改变槽位数量或页面结构，应新增模板 preset，而不是在 Theme Pack 中改几何。

## 素材规格

- Hero：展示 375 × 500，建议导出 1125 × 1500。
- 道具主体：展示约 59 × 72，建议透明 PNG/WebP 180 × 220，并留 8% 安全边。
- 普通奖励图：展示约 46 × 27，建议至少 140 × 82；留空时由模板绘制券框。
- 终极奖励图：展示约 44 × 30，建议至少 132 × 90。
- 奖励货架皮肤：展示 355 × 166，建议导出 1065 × 498。

默认夏日方案回填了当前活动页真实存在的 6 张道具图片和金马终极奖励图。
其余 3 张道具使用 Emoji 兜底，前三档优惠券使用模板实时样式；配置器会明确标注，
不会把不存在的文件伪装成图片素材。

## JSON 结构

每个导出的方案包含：

- `version`：配置版本
- `id / name / baseTheme`
- `content`：运营文案、卡片、任务、奖励与内容模块
- `pack.assets`：Hero、地图、奖励、按钮和卡框素材
- `pack.colors`：活动页使用的语义色 token
- `updatedAt`

上传的本地素材会以 Data URL 进入当前草稿，便于即时预览。大素材可能超过浏览器
本地存储容量，正式生产应将素材上传到对象存储，再在 JSON 中保存稳定 URL。
