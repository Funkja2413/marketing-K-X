...
- Hero 道具图层：卡片 ID、透明素材与在 `375 × 500` 坐标系中的落位。
...
- 活动场设计尺寸：`375 × 500`，只保留顶部圆角，底部为直角。
- Hero 媒体槽同样是完整的 `375 × 500`；顶部 `375 × 375` 是关键内容安全区。
- 底部实时按钮层固定在 Hero 的 `Y=375–425`，显示高度为 `375 × 50`，叠加在媒体与渐变层上。
- Hero 的 `Y=425–460` 是 `35px` 背景延伸区；奖励卡片从 `Y=433` 开始，与 Hero 错层重叠 `27px`。
- 图片标准导出尺寸：`1125 × 1500`（3×）；视频建议使用相同的 `3 : 4` 比例，并提供同尺寸 Poster。
- 现有 `1125 × 1125` 方图也可以继续使用：配置 `fit: "contain"` 和 `position: "center top"`，底部 `85px` 延展区由 Hero 底色和渐变层衔接。
- 标题、副标题、活动日期和主视觉建议在设计侧合成为一张图片。
- 默认使用 `cover` 填满圆角 Mask；横版素材如需完整展示，可配置 `fit: "contain"`。
- 关键人物、Logo 和文字应避开 Tab、右侧分享/规则入口以及底部主操作区。
- 渐变 Mask 位于媒体之上、按钮和所有实时 UI 之下，从 Hero `Y=300` 开始，并在 `Y=460` 过渡到 `colors.page`。
...
- 定位坐标使用 Hero 可见区 `375 × 500`，不是原始导出图的尺寸。
...
- 最终合成图可以放进 `finalReference` 作半透明校对，但集齐 7 种时仍由底图与 7 个图层实时合成。
...
- Hero 道具图层在固定 `375 × 500` 坐标系中的局部落位数据。
